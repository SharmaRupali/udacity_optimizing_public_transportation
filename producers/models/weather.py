...
class Weather(Producer):
    ...
    def run(self, month):
        ...
        logger.info("weather kafka proxy integration complete - continuing")
        resp = requests.post(
            f"{Weather.rest_proxy_url}/topics/{self.topic_name}", # What URL should be POSTed to?
            headers={"Content-Type": "application/vnd.kafka.avro.v2+json"}, # What Headers need to bet set?
            data=json.dumps(
                {
        ...
        try:
            ...
            logger.debug(f"An error occurred: {json.dumps(resp.json(), indent=2)}")
